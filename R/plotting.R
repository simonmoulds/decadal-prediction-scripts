...
myplotfun2 <- function(x) {
  ...
  x1 <- x %>% filter(model %in% "P") %>% `$`(msss)
  y1 <- x %>% filter(model %in% "PT") %>% `$`(msss)
  pval1 <- wilcox.test(y1, x1, alternative = "two.sided", paired = TRUE)$p.value

  x2 <- x %>% filter(model %in% "PT") %>% `$`(msss)
  y2 <- x %>% filter(model %in% "NAOPT") %>% `$`(msss)
  pval2 <- wilcox.test(y2, x2, alternative = "two.sided", paired = TRUE)$p.value

  x3 <- x %>% filter(model %in% "P") %>% `$`(msss)
  y3 <- x %>% filter(model %in% "NAOPT") %>% `$`(msss)
...
myplotfun22 <- function(x) {
  x_median <-
    x %>%
    group_by(subset, model) %>%
    summarize(md = median(msss), n = n()) %>%
    mutate(md_label = sprintf(md, fmt = "%#.3f"))
  p <- ggplot(x, aes(x = interaction(subset, model), y = msss)) +
    ## stat_boxplot(coef = NULL) +
...
myplotfun3 <- function(x) {
  ...
  p =
    ...
    geom_sf(
      data = x,
      aes(size = msss_diff, shape = model, fill = increase),
      alpha = 0.6
    ) +
    facet_wrap(. ~ period, ncol = 1) +
    coord_sf(
...
myplotfun444 <- function(x) {
  x_median <-
    x %>%
    group_by(model) %>%
    summarize(md = median(msss_diff), n = n()) %>%
    mutate(md_label = sprintf(md, fmt = "%#.3f"))
  p = ggplot(x, aes(x = model, y=msss_diff)) +
    ## stat_boxplot(coef = NULL) +
    geom_boxplot(
      lwd = 0.25,
      outlier.size = 0.25,
...
myplotfun9 <- function(x) {
  ## p = ggplot(x, aes(x = msss_diff, y=obs_msss, color = model)) +
  p = ggplot(x, aes(x = obs_msss, y = msss_diff, color = model)) +
    geom_point()  +
    scale_y_continuous(
      name="\u0394 MSSS (Predictions)",
      breaks=c(-0.8, -0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6, 0.8),
      limits=c(-0.8, 0.8)
    ) +
    scale_x_continuous(
      name="MSSS (Observations)",
